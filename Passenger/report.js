...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const continueBtn = document.querySelector(
    "#screen-picker button.border-black, #screen-picker button.border",
  );
  document
    .querySelector("#screen-picker")
  ...
  const cancelBtn = document.querySelector(
    "#screen-complaint button:first-of-type",
  );
  const submitBtn = document.querySelector(
    "#screen-complaint button:last-of-type",
  );

  // Find CANCEL and SUBMIT COMPLAINT properly
  document
    .querySelector("#screen-complaint")
    ?.querySelectorAll("button")
    .forEach((btn) => {
      ...
      if (btn.textContent.includes("SUBMIT COMPLAINT")) {
        btn.addEventListener("click", () => {
          ...
          if (!descriptionArea?.value.trim()) {
            ...
            if (!hasError)
              showToast(
                "Description is too short. Please provide more detail.",
                "error",
              );
            hasError = true;
          } else {
  ...
  const emergencyTextarea = document.querySelector(
    "#screen-emergency textarea",
  );

  document
    .querySelector("#screen-emergency")
    ?.querySelectorAll("button")
    .forEach((btn) => {
      // Back button
  ...
  emergencyTextarea?.addEventListener("input", () =>
    emergencyTextarea.classList.remove("border-red-500"),
  );
